-- ==========================================
-- CONSOLIDATED SCHEMA FOR SUPABASE SQL EDITOR
-- OFICINA HM AUTO ELÉTRICA SYSTEM
-- ==========================================
...
-- 3. PERMISSIONS AND SECURITY SCHEMA (app_private)
...
create policy "companies_write" on public.companies for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]));

create policy "clients_select" on public.clients for select to authenticated using (app_private.is_active_user(auth.uid()));
create policy "clients_write" on public.clients for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]));

create policy "vehicles_select" on public.vehicles for select to authenticated using (app_private.is_active_user(auth.uid()));
create policy "vehicles_write" on public.vehicles for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]));
...
create policy "service_orders_insert" on public.service_orders for insert to authenticated with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]));
create policy "service_orders_update" on public.service_orders for update to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]) or mechanic_id = auth.uid()) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]) or mechanic_id = auth.uid());
...
create policy "checklists_write" on public.checklists for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[]));
...
create policy "checklist_items_write" on public.checklist_items for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[]));

create policy "media_select" on public.media for select to authenticated using (app_private.is_active_user(auth.uid()));
create policy "media_insert" on public.media for insert to authenticated with check (app_private.is_active_user(auth.uid()) and created_by = auth.uid());
create policy "media_delete" on public.media for delete to authenticated using (created_by = auth.uid() or app_private.is_manager(auth.uid()));

create policy "quotes_select" on public.quotes for select to authenticated using (app_private.is_active_user(auth.uid()));
create policy "quotes_write" on public.quotes for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[]));
...
create policy "quote_items_write" on public.quote_items for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[]));
...
create policy "purchase_orders_write" on public.purchase_orders for all to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[])) with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria']::public.app_role[]));

create policy "approvals_select" on public.approvals for select to authenticated using (app_private.is_active_user(auth.uid()));
create policy "approvals_insert" on public.approvals for insert to authenticated with check (app_private.has_any_role(auth.uid(), array['dono','gerente','secretaria','mecanico']::public.app_role[]));
create policy "approvals_update" on public.approvals for update to authenticated using (app_private.has_role(auth.uid(), required_role)) with check (app_private.has_role(auth.uid(), required_role) and decided_by = auth.uid());

create policy "payments_select_financeiro" on public.payments for select to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','contabilidade','secretaria']::app_role[]));
create policy "payments_insert_financeiro" on public.payments for insert to authenticated with check (app_private.has_any_role(auth.uid(), array['dono','gerente','contabilidade','secretaria']::app_role[]));
create policy "payments_update_manager" on public.payments for update to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','contabilidade']::app_role[]));
create policy "payments_delete_owner" on public.payments for delete to authenticated using (app_private.has_role(auth.uid(), 'dono'::app_role));

create policy "expenses_select_financeiro" on public.expenses for select to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','contabilidade']::app_role[]));
create policy "expenses_write_financeiro" on public.expenses for insert to authenticated with check (app_private.has_any_role(auth.uid(), array['dono','gerente','contabilidade']::app_role[]));
create policy "expenses_update_financeiro" on public.expenses for update to authenticated using (app_private.has_any_role(auth.uid(), array['dono','gerente','contabilidade']::app_role[]));
create policy "expenses_delete_owner" on public.expenses for delete to authenticated using (app_private.has_role(auth.uid(), 'dono'::app_role));
...
create policy "oficina_media_select" on storage.objects for select to authenticated using (bucket_id = 'oficina-media' and app_private.is_active_user(auth.uid()));
create policy "oficina_media_insert" on storage.objects for insert to authenticated with check (bucket_id = 'oficina-media' and app_private.is_active_user(auth.uid()) and owner = auth.uid());
create policy "oficina_media_delete" on storage.objects for delete to authenticated using (bucket_id = 'oficina-media' and (owner = auth.uid() or app_private.is_manager(auth.uid())));

-- 7. SEED INITIAL PERMISSIONS AND STORAGE BUCKET
...
-- Enable supabase realtime for service_orders table
do $$ begin
  begin
